/*!
CUSTOM BUILD TO SUPPORT YOUTUBE AND VIMEO - DOWNLOADED FROM: https://github.com/jhurliman/video-js
...
_V_.extend({
  ...
  // BB CUSTOM
  isJQMobile: function(){ return !!$('div.ui-page-active').length; },
  iOSRescaleBugFix: function(className){
    // iFrame on iOS is buggy and must be scaled back to original size within a scaled container
    if (_V_.isJQMobile() && _V_.isIOS()){
      window.onorientationchange = function(){ _V_.iOSRescaleBugFix(className); };
      $(className).each(function(){
        var $this = $(this);
        var height = $this.parent().height();
        var width = $this.parent().width();
        var parentScale = parseFloat($this.closest('.profile-card').attr('style').match(/scale\(([01]?\.?\d+)\)/)[1]);
        var finalScale = 1 / parentScale;
        if (_V_.isIPhone()){
          // scaling back up
...
_V_.youtube = _V_.PlaybackTech.extend({
  ...
  loadApi: function() {
    ...
    // BB CUSTOM
    // had to add && YT because the script tag presence is NOT an indicator of the library being loaded (it's async)
    if (_V_.el(id) && typeof YT != 'undefined') {
...
window.onYouTubePlayerAPIReady = function() {
  ...
  // BB CUSTOM
  _V_.iOSRescaleBugFix('.video-youtube iframe');
  // END BB CUSTOM
};
...
_V_.vimeo = _V_.PlaybackTech.extend({
  ...
  loadPlayer: function(videoId){
    // BB CUSTOM
    var baseUrl = 'https://player.vimeo.com/video/';  // fix for production iframe ssl postMessage issue - always use https
    // END BB CUSTOM

    // ORIGINAL
    /* This is the original video-js vimeo support code which was found not to work fully behind https protocol as of 5/23/12
...
_V_.vimeo.onReady = function(id) {
  ...
  // BB CUSTOM
  _V_.iOSRescaleBugFix('.video-vimeo iframe');
  // END BB CUSTOM
};
...
var Froogaloop = (function(){
    ...
    function postMessage(method, params, target) {
        ...
        if (url.substr(0, 2) === '//') {
          ...
          // BB CUSTOM
          // always replace url protocol with secure - bug: vimeo iframe support always uses http: as of 5/23/12
          url = 'https:' + url.substr(6, url.length);
          // END BB CUSTOM
        }
...
_V_.BigPlayButton = _V_.Button.extend({
  ...
  onClick: function(){
    // Custom report google analytics for homepage video (need to investigate other ways to set a setting further up)
    if(this.player.options.poster == '/video/BizBuilt.png' && typeof _gaq != 'undefined' && window['_gaq'] != undefined){
      _gaq.push(['_trackEvent', 'Videos', 'Play', 'HomepageVideo']);
    }
...
_V_.LoadingSpinner = _V_.Component.extend({
  init: function(player, options){
    ...
    // hide spinner if we were 'seeking' to the beginning (as we do on 'ended')
    player.addEvent("timeupdate", _V_.proxy(this, function(){
